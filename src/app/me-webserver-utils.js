class PlcCommunication {
	...
	function writeBit(device, bit) {
		xhrBit = new XMLHttpRequest();
		xhrBit.open('POST', "/cgi/WrDev.cgi", true);
		xhrBit.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
		xhrBit.timeout = 3000;
		xhrBit.send('NUM=1&DEV1=' + device + '&TYP1=B&DATA='+bit);
	}
	function setBit(device){
		writeBit(device,1);
	}
	function resetBit(device){
		writeBit(device,0);
	}
	...
	updateValue(device,value){
		let index=this.plcRegs.findIndex(obj => {return obj.device === device});
		this.plcRegs[index].value=value;
	}
	readFromPLC(/* <String, {device, type, elementId}>[] */dataArray, callback, callbackError) {
		var obj=this;
		var xhr = new XMLHttpRequest();
		xhr.open('POST', "/cgi/RdDevRnd.cgi", true); // rnd blk 
		xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
		
		var func = function() {obj.callbackMain(xhr,obj,dataArray,callback,callbackError); }; //callback i callbackError sa podawane do funkcji
	...
	writeToPLC(dataArray, callback, callbackError) {
		...
		xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
		var FUNC = function() { obj.callbackMain(xhr,obj,dataArray,callback,callbackError); }; // Response analysis function setting
		xhr.onload = FUNC;
	...
	callbackMain(xhr,object,dataArray,callback,callbackError)
	{
	    ...
	    if ( 200 != xhr.status ) {
	        ...
	        callbackError("Error:"+"XMLHttpRequest.status="+xhr.status+" XMLHttpRequest.readyState="+xhr.readyState);
	        return;
	    }
	    var errorMessage;
	    ...
	    if( res.RET != "0000" ) {
	        ...
	        if( res.RET == "0001" ){errorMessage = "Not login!";document.location.href = "/system/Log-in.html"}
	        if( res.RET == "0002" ){errorMessage = "No permission (A user without device write permission executed the CGI.)";}
	        if( res.RET == "0005" ){errorMessage = "Incorrect request source (Referer)";}
	        if( res.RET == "4005" ){errorMessage = "Exceeded the number of points";}
	        if( res.RET == "4030" ){errorMessage = "Incorrect device type";}
	        if( res.RET == "4031" ){errorMessage = "Out of device range";}
	        if( res.RET == "4041" ){errorMessage = "Error due to the specified buffer memory number + specified number of transfer points out of buffer memory range";}
	        if( res.RET == "4043" ){errorMessage = "Error due to the specified module not existing";}
	        if( res.RET == "4080" ){errorMessage = "CGI parameter error";}
	...
	read()
	{
		this.readFromPLC(this.plcRegs, this.updateElements, this.updateElementsError);
		//console.log(plcRegs);
	}
	write(){ 
		...
		this.writeToPLC(this.plcRegs,this.updateElements,this.updateElementsError) 
	}
	
	getRequestStringRead(dataArray) {
		var str = "NUM=" + dataArray.length;
		for (let i = 0; i < dataArray.length; i++) {
			str += "&DEV" + Number(i+1) + "=" + dataArray[i].device + "&TYP" + Number(i+1) + "=" + this.MapPLC_TypeToSize.get(dataArray[i].type);
		}
		return str;
	}
	...
	getRequestStringWrite(dataArray) {
		...
		for (let i = 0; i < numberOfRegisters; i++) {
			...
			if (name=="X"||name=="Y"){ base=8;}
	        else if (name=="W"||name=="SW"){ base=16;}
	        else{ base=10;} //
			str += "&DEV" + Number(i+1) + "=" + name + number.toString(base) + "&TYP" + Number(i+1) + "=" + this.MapPLC_TypeToSize.get(registersToSend[i].type);
			str += '&DATA' + Number(i+1) + "=" + this.parsersToHexMap.get(registersToSend[i].type)(registersToSend[i].value);
		}
		for (let i = 0; i < dataArray.length; i++) {
